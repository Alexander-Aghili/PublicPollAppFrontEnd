...
class PollRequests {
  String url = 'http://192.168.87.92:8081/PublicPollBackEnd/publicpoll/poll/';
  /* GET Requests*/
  Future<Poll> getPollByID(String id) async {
    final response = await http.get(Uri.parse(url + 'getPoll/' + id));

    if (response.statusCode == 200) {
      print(Poll.fromJson(jsonDecode(response.body)));
    } else {
  ...
  Future<List<Poll>> getPolls() async {
    final response = await http.get(Uri.parse(url + 'getPolls'));
